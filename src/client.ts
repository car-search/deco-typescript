...
export interface ClientOptions {
  /**
   * Defaults to process.env['DECO_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['DECO_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['DECO_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Deco API. 
 */
export class Deco {
  apiKey: string | null;
   ...
   * API Client for interfacing with the Deco API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['DECO_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['DECO_BASE_URL'] ?? https://api.example.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DECO_BASE_URL'),
    apiKey = readEnv('DECO_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Deco.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('DECO_LOG'), 'process.env[\'DECO_LOG\']', this) ?? defaultLogLevel;
  ...
  static DecoError = Errors.DecoError;
